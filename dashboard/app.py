...
def _synthetic_customers(n: int = 7043) -> pd.DataFrame:
    ...
    segments = rng.choice(
        ["Champions", "Loyal Customers", "At Risk",
         "Hibernating", "Lost"],
        size=n, p=[0.20, 0.25, 0.25, 0.18, 0.12]
    )
...
def page_overview(df: pd.DataFrame, rev_df: pd.DataFrame):
    ...
    with col2:
        st.markdown('<div class="section-header">Churn by Contract Type</div>',
                    unsafe_allow_html=True)
        churn_contract = (
            df.groupby("contract_type")["label"]
            .agg(["sum", "count"])
            .reset_index()
        )
    ...
    with col4:
        st.markdown('<div class="section-header">Churn Rate by Tenure Band</div>',
                    unsafe_allow_html=True)
        df2 = df.copy()
        ...
        tenure_churn = (
            df2.groupby("tenure_band", observed=True)["label"]
            .mean()
            .reset_index()
        )
        tenure_churn["churn_pct"] = (tenure_churn["label"] * 100).round(1)
        fig4 = px.bar(
            tenure_churn, x="tenure_band", y="churn_pct",
            color="churn_pct",
            color_continuous_scale=["#00c851", "#ff8800", "#ff4444"],
            template="plotly_dark",
            text="churn_pct",
...
def page_churn_analysis(df: pd.DataFrame):
    ...
    with col1:
        ...
        if "churn_prob" not in df.columns:
            df["churn_prob"] = df["label"].apply(
                lambda x: np.random.default_rng(x).uniform(0.6, 0.9)
                if x == 1 else np.random.default_rng(x).uniform(0.05, 0.4)
            )
        fig = px.histogram(
            df, x="churn_prob", color="risk_tier",
    ...
    with col2:
        st.markdown('<div class="section-header">Monthly Charges vs Churn Risk</div>',
                    unsafe_allow_html=True)
    ...
    st.markdown('<div class="section-header">🚨 High Risk Customers — Requires Action</div>',
                unsafe_allow_html=True)
    high_risk = (
    ...
    display_cols = [
        ...
        "contract_type", "total_support_tickets",
...
def page_revenue(rev_df: pd.DataFrame):
    ...
    growth   = (fc_df["forecast"].iloc[-1] - fc_df["forecast"].iloc[0]) / \
               fc_df["forecast"].iloc[0] * 100
    ...
    st.markdown("<br/>", unsafe_allow_html=True)
    st.markdown('<div class="section-header">Historical + Forecast Revenue</div>',
                unsafe_allow_html=True)
...
def page_segmentation(df: pd.DataFrame):
    ...
    with col2:
        st.markdown('<div class="section-header">Avg Monthly Charges by Segment</div>',
                    unsafe_allow_html=True)
    ...
    st.markdown('<div class="section-header">Segment Profiles</div>',
                unsafe_allow_html=True)
    ...
    seg_profile.columns = ["Segment", "Customers", "Avg Tenure (mo)",
                           "Avg Monthly Charge", "Churn Rate", "Avg Support Tickets"]
...
def page_live_predictor():
    ...
    if submitted:
        try:
            ...
            with col_a:
                color = "#ff4444" if tier == "HIGH" else \
                        "#ff8800" if tier == "MEDIUM" else "#00c851"
                st.markdown(f"""
...
def page_monitoring():
    ...
    with col1:
        st.markdown('<div class="section-header">AUC-ROC Over Time</div>',
                    unsafe_allow_html=True)
    ...
    with col2:
        st.markdown('<div class="section-header">Feature Drift (PSI) by Feature</div>',
                    unsafe_allow_html=True)
        features = [
            "monthly_charges", "tenure_months", "total_charges",
            "support_tickets", "charge_gap", "services_count",
        ]
        ...
        fig2 = px.bar(
            ...
            x="psi", y="feature", color="status",
            orientation="h",
            color_discrete_map=colors,
            template="plotly_dark",
        )
        fig2.add_vline(
            x=0.20, line_dash="dash",
            line_color="#ff4444",
