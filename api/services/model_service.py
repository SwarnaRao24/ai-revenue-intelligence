...
class ModelService:
    ...
    def _build_churn_row(self, f: dict) -> dict:
        ...
        return {
            ...
            "is_monthly_contract":   1 if f.get("contract_type") == "Month-to-month" else 0,
            "is_annual_contract":    1 if f.get("contract_type") == "One year" else 0,
            "is_biannual_contract":  1 if f.get("contract_type") == "Two year" else 0,
            "uses_electronic_check": 1 if f.get("payment_method") == "Electronic check" else 0,
            ...
            "is_at_risk":            1 if (f.get("contract_type") == "Month-to-month" and tenure < 12) else 0,
            ...
            "negative_ticket_ratio": (max(0, tickets - 1) / tickets) if tickets > 0 else 0,
    ...
    def predict_revenue(self, periods: int) -> dict:
        ...
        trend_str = (
            "growing" if values[-1] > values[0] * 1.05
            else "declining" if values[-1] < values[0] * 0.95
            else "stable"
        )
    ...
    def assess_risk(self, features: dict) -> dict:
        ...
        recs = [f["feature"].replace("_", " ").title()
