...
def _engineer_churn_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    feat["monthly_charges"] = pd.to_numeric(
    ...
    feat["uses_electronic_check"] = (df["paymentmethod"] == "Electronic check").astype(
        int
    )
    ...
    feat["services_count"] = (
        feat["has_online_security"]
        + feat["has_tech_support"]
        + feat["has_online_backup"]
        + feat["has_device_protect"]
        + feat["has_streaming_tv"]
        + feat["has_streaming_movies"]
    )
    feat["is_high_value"] = (feat["monthly_charges"] > 70).astype(int)
    feat["is_at_risk"] = (
        (feat["is_monthly_contract"] == 1) & (feat["tenure_months"] < 12)
    ).astype(int)
    ...
    feat["lifetime_value"] = (
        feat["monthly_charges"] * feat["tenure_months"] + feat["total_revenue_6mo"]
    ).round(2)
    ...
    feat["negative_ticket_ratio"] = (
        feat["negative_support_tickets"] / feat["total_support_tickets"].replace(0, 1)
    ).round(3)
    feat["charge_to_ltv_ratio"] = (
        (feat["monthly_charges"] / feat["lifetime_value"].replace(0, np.nan))
        .fillna(0)
        .round(4)
    )
...
def _generate_synthetic_churn_features(n: int = 7043) -> pd.DataFrame:
    ...
    df.loc[churn, "internet_service"] = rng.choice(
        ["Fiber optic", "DSL"], size=churn.sum(), p=[0.75, 0.25]
    )
